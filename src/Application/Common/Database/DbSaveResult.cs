...
{
    private const string IndexNamePrefix = "IX_"; //Potential Postgres Dependency!!!
    ...
    public static DbSaveResult FromSuccess(int affectedRows)
    {
        return new DbSaveResult(DbStatus.Success, affectedRows, null);
    }

    public static DbSaveResult FromError(DbStatus status, string? constraintName)
    {
        return new DbSaveResult(status, 0, constraintName);
    }
